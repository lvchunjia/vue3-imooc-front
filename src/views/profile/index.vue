 ...
 * 数据本地的双向同步
 */
const loaclUserInfo = ref({ ...userInfo.value })
...
const onChangeProfile = async () => {
  ...
  setUserInfo(loaclUserInfo.value)
  loading.value = false
}
...
<template>
  <div class="h-full bg-zinc-200 dark:bg-zinc-800 duration-400 overflow-auto xl:pt-1">
    ...
    >
      <!-- 移动端 navbar -->
      <m-navbar sticky v-if="isMobile" :clickLeft="onNavbarLeftClick"> 个人资料 </m-navbar>
      <!-- pc 端 -->
      ...
      <div class="h-full w-full px-1 pb-4 text-sm mt-2 xl:w-2/3 xl:pb-0">
        ...
        <div class="py-1 xl:flex xl:items-center xl:my-1">
          ...
          <m-input v-model="loaclUserInfo.nickname" class="w-full" type="text" max="20"></m-input>
        </div>

        <!-- 职位 -->
        <div class="py-1 xl:flex xl:items-center xl:my-1">
          <span class="w-8 block mb-1 font-bold dark:text-zinc-300 xl:mb-0">职位</span>
          <m-input v-model="loaclUserInfo.title" class="w-full" type="text"></m-input>
        </div>

        <!-- 公司 -->
        <div class="py-1 xl:flex xl:items-center xl:my-1">
          <span class="w-8 block mb-1 font-bold dark:text-zinc-300 xl:mb-0">公司</span>
          <m-input v-model="loaclUserInfo.company" class="w-full" type="text"></m-input>
        </div>

        <!-- 个人主页 -->
        <div class="py-1 xl:flex xl:items-center xl:my-1">
          <span class="w-8 block mb-1 font-bold dark:text-zinc-300 xl:mb-0">个人主页</span>
          <m-input v-model="loaclUserInfo.homePage" class="w-full" type="text"></m-input>
        </div>
        ...
        <div class="py-1 xl:flex xl:my-1">
          ...
          <m-input
            v-model="loaclUserInfo.introduction"
            class="w-full"
            type="textarea"
            max="50"
          ></m-input>
